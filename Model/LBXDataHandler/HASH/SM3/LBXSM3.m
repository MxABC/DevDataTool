 ...
 */
static int sm3_file( const char *path, unsigned char output[32] )
{
    FILE *f;
    size_t n;
    sm3_context ctx;
...
+ (NSData*)sm3WithData:(NSData*)data
{
    ...
    UInt8 output[32]={0};
    sm3((uint8_t*)data.bytes, (uint32_t)data.length, output);
...
+ (NSData*)HMACSM3WithKey:(NSData*)key data:(NSData*)data
{
    ...
    sm3_hmac((uint8_t*)key.bytes, (uint32_t)key.length, (uint8_t*)data.bytes, (uint32_t)data.length, output);
...
+ (NSData*)sm3WithFilePath:(NSString*)filePath
{
    ...
    int ret = sm3_file([filePath UTF8String],output);
