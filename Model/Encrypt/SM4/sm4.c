...
static void sm4_2_setkey( unsigned long SK[32], unsigned char key[16] )
{
    unsigned long MK[4];
    unsigned long k[36];
    unsigned long i = 0;
 ...
 *
 */
static void sm4_2_one_round( unsigned long sk[32],
...
{
    ctx->mode = sm4_2_ENCRYPT;
	sm4_2_setkey( ctx->sk, key );
}
                   ...
                   unsigned char *output)
{
    while( length > 0 )
    {
        sm4_2_one_round( ctx->sk, input, output );
        input  += 16;
        output += 16;
        length -= 16;
    }
                    ...
                    unsigned char *output )
{
    ...
    if( mode == sm4_2_ENCRYPT )
    {
        while( length > 0 )
        {
            ...
            sm4_2_one_round( ctx->sk, output, output );
            memcpy( iv, output, 16 );
